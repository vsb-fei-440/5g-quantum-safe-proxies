...
RESULTS_DIR = "results"
MEASURE_ASSUME_KME_BUFFERED = False  
MEASURE_MULTIPLIER = 1             

# ----- KMS endpoints -----
KMS_ENC_IP = "10.250.0.2"  
KMS_DEC_IP = "10.250.1.2"  
KMS_PORT = 80
...
UDP_SWITCH_BYTES = 10 * 1024 * 1024        
TCP1_SWITCH_BYTES = 2 * 1024 * 1024  
TCP2_SWITCH_BYTES = 2 * 1024 * 1024  
...
# ----- OpenSSL behavior -----
# If you are using a specific PQC build, change this to e.g., "/usr/local/openssl-3.5/bin/openssl"
OPENSSL_BIN = "openssl" 
OPENSSL_FORCE_TLS13 = True
OPENSSL_TLS13_CIPHERSUITES = ""  

# ===== PKI / Cert Fallback Configuration =====
ENABLE_CERT_FALLBACK = True  # Toggle Fallback feature
CERTS_DIR = "certs_ml"
PQC_KEM = "mlkem768"

def get_cert_paths(role: str) -> dict:
...
def _openssl_s_client_cert(host: str, port: int) -> subprocess.Popen:
    certs = get_cert_paths("sae")
    cmd = [
        OPENSSL_BIN, "s_client",
        "-connect", f"{host}:{port}",
        "-quiet",
...
def udp_frame_read_from_pipe(pipe, timeout: float = 0.1) -> Optional[bytes]:
    ...
    if ln <= 0 or ln > (UDP_MAX_PAYLOAD * 8): return None
...
def _sae_commit_initial_service(service: str, kid: str, port_a: int, port_b: int) -> bool:
    op_prep = "prepare_fallback" if kid == "FALLBACK" else "prepare"
    op_com = "commit_fallback" if kid == "FALLBACK" else "commit"
    
    if not keysync_post({"op": op_prep, "service": service, "key_ID": kid, "port": port_a}): return False
    return keysync_post({"op": op_com, "service": service, "key_ID": kid, "new_port": port_a, "old_port": port_b, "grace": 0.0})

def _sae_rotate_service(state: SaeState, svc: SaeServiceState, grace: float) -> bool:
    km = state.pool.pop_one()
    
    if not km:
        if not ENABLE_CERT_FALLBACK:
            return False
        if svc.key_id == "FALLBACK":
            return False  # Already in fallback, keep it running
        new_kid = "FALLBACK"
        new_psk = None
    else:
        new_kid, new_psk = km
    ...
    op_prep = "prepare_fallback" if new_kid == "FALLBACK" else "prepare"
    op_com = "commit_fallback" if new_kid == "FALLBACK" else "commit"

    if not keysync_post({"op": op_prep, "service": svc.name, "key_ID": new_kid, "port": new_port}):
        if km: state.pool.push_front(km)
        return False

    if not keysync_post({"op": op_com, "service": svc.name, "key_ID": new_kid, "new_port": new_port, "old_port": old_port, "grace": grace}):
        if km: state.pool.push_front(km)
        return False
    ...
    if state.meas is not None and new_kid != "FALLBACK":
        state.meas.record_key_pop(svc.name)
        
    mode_str = "PKI" if new_kid == "FALLBACK" else "PSK"
    _log("SAE-KMS", f"{svc.name.upper()} Switched to {mode_str} key_ID={new_kid} -> port={new_port}; sae_pool={state.pool.size()}")
    return True
...
def sae_udp_restart_client(state: SaeState) -> None:
    with state._udp_client_lock:
        ...
        if not state.udp.key_id or (state.udp.key_id != "FALLBACK" and not state.udp.psk_hex): 
            return
        ...
        mode_str = "PKI/CERT" if kid == "FALLBACK" else "PSK"
        _log("SAE-TLS", f"UDP persistent s_client connecting ({mode_str}) to {KME_HOST}:{port} key_ID={kid}")
        
        deadline = time.time() + SAE_TCP_TLS_CONNECT_RETRY_SEC
        p = None
        while time.time() < deadline and not state.stop.is_set():
            if kid == "FALLBACK":
                p = _openssl_s_client_cert(KME_HOST, port)
            else:
...
def sae_udp_rotator(state: SaeState) -> None:
    while not state.stop.is_set() and state.udp.key_id is None:
        ...
        if state.meas is not None and kid != "FALLBACK":
...
def _sae_tcp_listener_loop(
    ...
    while not state.stop.is_set():
        try:
            ...
            if not svc.key_id or (svc.key_id != "FALLBACK" and not svc.psk_hex):
                cs.close()
                continue
...
class KeySyncHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        st: KmeState = self.server.kme_state  
        op = obj.get("op", "")
        ...
        if op in ("prepare", "prepare_fallback"):
            kid = obj.get("key_ID", "")
            port = int(obj.get("port", 0))
            if not kid or port <= 0:
                _json_ok(self, 400, {"ok": False, "error": "missing key_ID/port"})
                return
            if op == "prepare" and not _kme_ensure_dec_key(st, kid):
                _json_ok(self, 503, {"ok": False, "error": "dec_key unavailable"})
                return
            _json_ok(self, 200, {"ok": True, "op": op})
            return

        if op in ("commit", "commit_fallback"):
            kid = obj.get("key_ID", "")
            ...
            if not kid or new_port <= 0 or old_port <= 0:
                ...
                return
            if op == "commit" and not _kme_ensure_dec_key(st, kid):
                _json_ok(self, 503, {"ok": False, "error": "dec_key unavailable"})
                return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="QKD-KMS TLS proxy (UDP + 2x TCP) with alternating TLS ports and PKI Fallback.")
